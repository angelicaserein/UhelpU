...
```
js/key-binding-system/
├── KeyBindingConfig.js        - localStorage 读写
└── KeyBindingManager.js       - 单例管理器

js/character-control-system/
├── EventProcesser.js          - 按键过滤与事件转发
└── IntentResolver.js          - keyCode → intent 映射

js/ui/windows/
└── WindowSetting.js           - 设置窗口中的按键改绑 UI

css/style.css                  - 按键设置相关样式
js/i18n/index.js               - 文案翻译键
```
...
if (event.type === "keydown") {
  ...
  } else if (mappedIntent === "dash") {
    // ← 新增
    intent.add("wantsDash");
  } else if (mappedIntent === "attack") {
    // ← 新增
