...
export class WireIndicatorSystem {
  /**
   * @param {Array} buttonList - array of button objects (need isPressed property) | 按鹁对象数组（需要 isPressed 属性）
   * @param {Array} wirePaths - wire path points for each route [[{x,y},...], ...] | 每路电线的路径点数组
   * @param {Array} indicatorPoints - indicator light coordinates [{x,y}, ...] | 指示灯坐标数组
   * @param {Function} onAllArrived - callback when all arrived | 全部到达时的回调
   * @param {Object} [options] - optional: wireSpeed, indicatorColors | 可选项：wireSpeed, indicatorColors
   */
  constructor(
    buttonList,
    wirePaths,
  ...
  update() {
    for (let i = 0; i < this.buttons.length; i++) {
      if (this.buttons[i].isPressed) {
        this._wireProgress[i] = Math.min(
          1,
          this._wireProgress[i] + this.wireSpeed,
        );
      } else if (!this._locked) {
        this._wireProgress[i] = 0;
      }
    }
    // Indicator light state | 指示灯状态
    for (let i = 0; i < this._indicatorOn.length; i++) {
      this._indicatorOn[i] = this._wireProgress[i] >= 1.0 || this._locked;
    }
    // All arrived and not locked, trigger callback | 全部到达且未锁定时，触发回调
