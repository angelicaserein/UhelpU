...
export class WindowPause extends WindowBase {
  ...
  constructor(p, callbacks = {}) {
    ...
    // 启用键盘导航（重要：必须在 _buildContent 后调用）
    this.enableKeyboardNavigation(
      [
        ...
        },
        {
          ...
          callback: () => {
            this.close();
            if (this._callbacks.onRestartLevel)
              this._callbacks.onRestartLevel();
          },
        },
        {
          btn: this._levelChoiceBtn,
          callback: () => {
            this.close();
            if (this._callbacks.onBackLevelChoice)
              this._callbacks.onBackLevelChoice();
          },
        },
        {
          btn: this._menuBtn,
  ...
  _refreshLabels() {
    ...
    this._levelChoiceBtn &&
      this._levelChoiceBtn.html(t("pause_back_level_choice"));
